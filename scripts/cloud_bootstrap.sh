#!/usr/bin/env bash
# One-shot bootstrap for a fresh RunPod pod.
#
# Brings a clean Ubuntu image to a state where ``scripts/cloud_run.sh``
# or ``scripts/cloud_decompose.sh`` can be executed. Idempotent — safe
# to re-run on a pod that has already been bootstrapped.
#
...
# What this does
# --------------
# 1. Verifies ``nvidia-smi`` is present and reports a GPU (skipped with --cpu).
...
# 3   uv sync failed
# 4   preflight failed (torch / HF auth)
...
if [[ "$CPU_ONLY" -eq 1 ]]; then
    log "CPU-only mode: skipping nvidia-smi / GPU checks."
else
    log "Checking for CUDA GPU…"
    if ! command -v nvidia-smi >/dev/null 2>&1; then
        fail "nvidia-smi not found. Is this actually a GPU pod? (Use --cpu for CPU pods.)" 2
    fi
...
# Keep uv caches on /workspace so the small container root disk does not fill.
export UV_CACHE_DIR="${UV_CACHE_DIR:-/workspace/.cache/uv}"
export UV_TOOL_DIR="${UV_TOOL_DIR:-/workspace/.uv/tools}"
...
if torch.cuda.is_available():
    ...
    print("Running CPU-only torch.")

# bitsandbytes is required for 4-bit quantization paths; skip import errors on CPU pods.
try:
...
if [[ "$CPU_ONLY" -eq 1 ]]; then
    ...
    log "Next: bash scripts/cloud_run.sh --model llama31_8b --emotions 'admiration joy loathing sadness'"
fi
