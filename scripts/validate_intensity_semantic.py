...
def main() -> int:
    ...
    # readout activation per row
    print(f"reading {len(rows)} templates at layer {layer} ({args.device}/{args.dtype}) ...")
    with torch.no_grad():
        try:
            for i, r in enumerate(rows, 1):
                ...
                lm.model(**inp)
                r["_act"] = rec.activations[layer][0].float().cpu().numpy().astype(np.float64)
                if i % 25 == 0 or i == len(rows):
                    print(f"  {i}/{len(rows)}", flush=True)
        finally:
            rec.remove()

    # group by (emotion, family)
    ...
    lines = [
        ...
        f"- Read-out: Assistant-colon projection onto the emotion vector; Spearman vs rank and vs raw x",
        f"- Target: ρ(proj, intensity_rank) ≥ {RANK_PASS}; on INVERSE families ρ(rank) ≫ ρ(x)",
        "",
        "| emotion | family | dir | n | ρ(proj, rank) | ρ(proj, raw x) |",
        "|---|---|---|---|---|---|",
    ]
    # Track inverse families (confound-controlled) separately from all families.
    per_emo_inv: dict[str, list[float]] = defaultdict(list)
    per_emo_all: dict[str, list[float]] = defaultdict(list)
    for (emotion, family), items in sorted(fam.items()):
        if emotion == "neutral":
            continue
        items.sort(key=lambda r: r["x"])
        proj = [float(r["_act"] @ vec[emotion]) for r in items]
        rho_rank = _spear(proj, [r["intensity_rank"] for r in items])
        rho_x = _spear(proj, [r["x"] for r in items])
        direction = items[0]["direction"]
        per_emo_all[emotion].append(rho_rank)
        if direction == "decreasing":
            per_emo_inv[emotion].append(rho_rank)
        mark = " **(inverse)**" if direction == "decreasing" else ""
        lines.append(
            f"| {emotion} | {family}{mark} | {direction[:3]} | {len(items)} "
    ...
    # Headline: INVERSE families only. On increasing families ρ(rank) == ρ(raw x)
    # by construction, so they cannot separate meaning from the digit; only the
    # inverse families do. Averaging them in (the old per-emotion mean) is
    # number-confounded — reported as secondary, not the verdict.
    lines += [
        "",
        "## Semantic intensity — INVERSE families only (confound-controlled)",
        "",
        f"On inverse families the number runs *against* the intended intensity, so "
    ...
    # neutral control: project neutral templates onto each emotion vector; the
    # number should NOT drive any emotion projection. Report the MEAN — max over
    # ~a dozen n=6 correlations is ~1.0 by chance, so mean is the honest summary.
    ...
    for (emotion, family), items in fam.items():
        ...
        for e in EMOTIONS:
            neutral_rho.append(abs(_spear([float(r["_act"] @ vec[e]) for r in items], xs)))
    if neutral_rho:
        lines += [
            "",
